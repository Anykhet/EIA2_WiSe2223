...
namespace L06_shoppinglist {
    ...
    async function handleload(): Promise<void> {
        ...
        let response: Response = await fetch("https://webuser.hs-furtwangen.de/~khetarpa/Database/index.php/?command=find&collection=data");
   ...
   // Funktion fürs einfügen neuer Einträge in die Liste
    ...
    async function daterefresh(nextelement: HTMLDivElement, product: string, amount: number, comment: string, nextpurchase: string, counter: number): Promise<void> {
        ...
        let response: Response = await fetch("https://webuser.hs-furtwangen.de/~khetarpa/Database/index.php?" + query.toString()); //DBS empfängt 
